...
La conception de ce cours a bénéficié de l'expérience conjointe du chercheur et du praticien. L'auteur exprime sa reconnaissance au \textbf{Professeur Flavien Serge Mani Onana} et au \textbf{Professeur Thomas Djotio Ndié} pour leur encadrement exigeant et leur ouverture d'esprit face à des travaux qui, de la cryptographie institutionnelle à la gouvernance des SI, explorent des territoires disciplinaires encore peu balisés.

La matière de ce cours a été nourrie par les missions menées au sein des \textbf{Services du Contrôle Supérieur de l'État}, où l'audit n'est pas une abstraction académique mais une responsabilité institutionnelle quotidienne. Elle a également été enrichie par les défis posés par le \textbf{projet GIP Bénin}, qui confronte en temps réel les principes de sécurité aux contraintes de la gouvernance publique africaine.

À nos étudiants, qui jouent le jeu de la simulation de crise avec un sérieux et une créativité qui ne cessent de surprendre.
...
\input{chapitres/03_gouvernance}
\input{chapitres/04_vocabulaire_risque}
\input{chapitres/05_referentiels}
\input{chapitres/06_application_risque}
\input{chapitres/07_traitement_reporting}
\input{chapitres/08_eval_module1}
...
\input{chapitres/09_intro_audit}
\input{chapitres/10_processus_audit_a}
\input{chapitres/11_processus_audit_b}
\input{chapitres/12_referentiels_audit}
\input{chapitres/13_application_audit}
\input{chapitres/14_rapport_audit}
\input{chapitres/15_eval_module2}
...
\input{chapitres/16_intro_incidents}
\input{chapitres/17_processus_incident}
\input{chapitres/18_forensics}
\input{chapitres/19_continuite}
\input{chapitres/20_application_pca}
\input{chapitres/21_eval_module3}
...
\input{chapitres/22_intro_crise}
\input{chapitres/23_cellule_crise}
\input{chapitres/24_communication_crise}
\input{chapitres/25_simulation_crise}
\input{chapitres/26_retex}
\input{chapitres/27_eval_module4}

% --- Résumé et Évaluation Finale ---
\input{chapitres/28_resume}
\input{chapitres/29_eval_finale}
